...
function invierArray($aut){
 ...
 if (count($aut) < 10 || count($aut[0]) < 12) {
    echo "El array no tiene las dimensiones esperadas (10 filas y 12 columnas).<br>";
    return;
  }
  ...
  $filas = array_slice($aut, -5);  // Slicing de las últimas 5 filas
  
  // Filtrar columnas 6 a 8
  foreach ($filas as $fila) {
    $nuevaFila = array_slice($fila, 5, 3);  // Slicing de las columnas 6 a 8 (índices 5, 6, 7)
